...
def test_pickling(free_port, signed_client, signed_server, ssl_contexts: tuple[ssl.SSLContext, ssl.SSLContext]):
    import pickle
    from hydra.distributed_queues.sink_queue import SinkQueueConsumer, SinkQueueFeed
    from hydra.distributed_queues.configuration import SSLCertificatesConfig
    SinkQueueFeed._pickle_counter = 0  # reset counter for test predictability
    localhost = '127.0.0.1'
    server_pem, server_key, ca_pem = signed_server
    client_pem, client_key, _ = signed_client
    server_ssl_context, client_ssl_context = ssl_contexts if ssl_contexts else (None, None)
    ...
    with SinkQueueConsumer[int, str](address=(localhost, free_port), sentinel="done", size=10,
                                                ssl_context=client_ssl_context).start(server_ssl_context) as server_queue:
        with SinkQueueFeed[int](name="pytest_client", address=(localhost, free_port), sentinel="done",
                                         ssl_context=client_ssl_context) as client_queue:
            with pytest.raises(PickleError):
                pickle.dumps(server_queue)
            data = pickle.dumps(client_queue)
            new_client_queue: SinkQueueFeed
            new_client_queue2: SinkQueueFeed
            with pickle.loads(data) as new_client_queue, \
                ...
                new_client_queue.put(1)
                assert server_queue.get() == 1

        server_queue.join(timeout=5)